...
function applyActivityFilter(value){const slug=String(value||'').trim().toLowerCase();if(!slug)return;state.activity=slug;state.q='';state.cat='Todas';$('#searchInput').value='';renderNav();$('#following').classList.add('hidden');$('#feedSection').classList.remove('hidden');$('#hero').classList.add('hidden');$('#topicsSection').classList.add('hidden');const welcome=document.querySelector('.welcome');if(welcome)welcome.classList.add('hidden');renderFeed();const count=filtered().length;const label=activityLabel(slug);$('#feedTitle').textContent=label;$('#feedSub').textContent=count?`${count} ${count===1?'contenido':'contenidos'} para esta actividad`:'Aún no hay contenidos para esta actividad';const feed=document.getElementById('feedSection');if(feed)feed.scrollIntoView({behavior:'smooth',block:'start'})}
