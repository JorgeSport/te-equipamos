...
function scrollActivityResultsStart(){requestAnimationFrame(()=>requestAnimationFrame(()=>{const first=document.querySelector('#feed .card');if(!first)return;const sticky=document.querySelector('.top');const responsive=window.matchMedia('(max-width:1100px)').matches;const offset=responsive?((sticky?sticky.getBoundingClientRect().height:0)+12):24;const y=Math.max(0,window.scrollY+first.getBoundingClientRect().top-offset);window.scrollTo({top:y,behavior:'smooth'})}))}
function applyActivityFilter(value){const slug=String(value||'').trim().toLowerCase();if(!slug)return;state.activity=slug;state.q='';state.cat='Todas';$('#searchInput').value='';renderNav();$('#following').classList.add('hidden');$('#feedSection').classList.remove('hidden');$('#hero').classList.add('hidden');$('#topicsSection').classList.add('hidden');const welcome=document.querySelector('.welcome');if(welcome)welcome.classList.add('hidden');renderFeed();const count=filtered().length;const label=activityLabel(slug);$('#feedTitle').textContent=label;$('#feedSub').textContent=count?`${count} ${count===1?'contenido':'contenidos'} para esta actividad`:'Aún no hay contenidos para esta actividad';scrollActivityResultsStart()}
function setCat(c){state.cat=c;state.q='';state.activity='';$('#searchInput').value='';renderNav();const home=c==='Todas';const intro=$('#portalIntro');const welcome=document.querySelector('.welcome');if(intro)intro.classList.add('hidden');if(welcome)welcome.classList.toggle('hidden',!home);$('#topicsSection').classList.add('hidden');if(c==='Siguiendo'){$('#following').classList.remove('hidden');$('#feedSection').classList.add('hidden');$('#hero').classList.add('hidden');renderFollowing()}else{$('#following').classList.add('hidden');$('#feedSection').classList.remove('hidden');$('#hero').classList.toggle('hidden',!home);renderFeed();updateSectionHead(c)}closeMenu();scrollTo({top:0,behavior:'smooth'})}"""
