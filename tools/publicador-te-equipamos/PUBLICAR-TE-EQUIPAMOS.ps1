...
function Test-StrategicManifest([string]$ManifestPath, [string]$ExpectedRepoName) {
    ...
    $productType = [string](Get-JsonPropertyValue $item "product_type")
    ...
    if ([string]::IsNullOrWhiteSpace($productType) -or $productType -match 'REEMPLAZAR_') {
        Stop-Publish "Falta product_type. Debe indicar la categoria principal del producto sobre la que trata el contenido."
    }
    $normalizedProductType = $productType.Trim().ToLowerInvariant()
    if ($normalizedProductType -match '^(editorial|review|noticia|news)([-_ ].*)?        Stop-Publish "Falta el titulo estrategico del Hub (card_title). Crealo antes de publicar."
    }
...
try {
    ...
    if ($tempRoot -and (Test-Path -LiteralPath $tempRoot)) {
        ...
        Stop-Publish "product_type debe describir la categoria principal del producto (por ejemplo: mochila, sandalias, zapatillas), no el formato editorial."
    }
